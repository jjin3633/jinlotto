...
def test_gsc_specific_issues():
    ...
    # 1. 사이트맵 크기 확인
    sitemap_response = requests.get('https://stretchinglotto.motiphysio.com/sitemap.xml')
    rss_response = requests.get('https://stretchinglotto.motiphysio.com/rss.xml')
    
    print(f"📄 사이트맵 크기: {len(sitemap_response.content)} bytes")
    print(f"📡 RSS 크기: {len(rss_response.content)} bytes")
    
    # 2. robots.txt에서 사이트맵 확인
    robots_response = requests.get('https://stretchinglotto.motiphysio.com/robots.txt')
